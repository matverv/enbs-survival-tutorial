...
plot_km_fun <- function (dataset, strategies, x_max, break_x, y_lab = "Survival probability", x_lab = "Time", ...) {
  ...
  fit_obj <- do.call(survfit, list(formula = Surv(times, event) ~ treat, data = dataset))
...
survfit_fun <- function (dists, 
  ...
  for (i in 1:length(dists)) {
    ...
    if(dists[i] == "exp" || dists[i] == "gengamma") {
      ...
      model <- flexsurvreg(Surv(times, event) ~ 1, data = ipd, 
...
run_pa_pembro <- function(nsim, price = 1) {
  ...
  ipd_os$times <- ipd_os$times/month2year*year2week
  ...
  # fit survival models
  smod_os_pembro <- survfit_fun(dist_os_pembro, subset(ipd_os, ipd_os$treat == 2), t_horizon)
  smod_os_suni <- survfit_fun(dist_os_suni, subset(ipd_os, ipd_os$treat == 1), t_horizon)
  ...
  ipd_pfs_pembro <- subset(ipd_pfs, ipd_pfs$treat == 2)
  ipd_pfs_suni <- subset(ipd_pfs, ipd_pfs$treat == 1)
  
  # bootstrap the row numbers of the PFS datasets
  samp_mat_pembro <- t(matrix(sample(1:length(ipd_pfs_pembro$times), length(ipd_pfs_pembro$times)*n_sim, replace=T),
                              ncol=length(ipd_pfs_pembro$times), byrow=T))
  
  samp_mat_suni <- t(matrix(sample(1:length(ipd_pfs_suni$times), length(ipd_pfs_suni$times)*n_sim, replace=T),
                            ncol=length(ipd_pfs_suni$times), byrow=T))
  
  # generate bootstrapped KM estimates for week 0:13
  km_pfs_pembro <- apply(samp_mat_pembro, 2, function (x) {
    summary(survfit(formula = Surv(times, event) ~ 1, data = ipd_pfs_pembro[x,]), time = c(0:13))$surv
  })
  
  km_pfs_suni <- apply(samp_mat_suni, 2, function (x) {
    summary(survfit(formula = Surv(times, event) ~ 1, data = ipd_pfs_suni[x,]), time = c(0:13))$surv
  })
  ...
  # fit survival models 
  smod_pfs_pembro <- survfit_fun(dist_pfs_pembro, subset(ipd_pfs, ipd_pfs$treat == 2), t_horizon)
  smod_pfs_suni <- survfit_fun(dist_pfs_suni, subset(ipd_pfs, ipd_pfs$treat == 1), t_horizon)
  ...
  smod_pfs_pembro <- survfit_fun("weibull", subset(ipd_pfs, ipd_os$treat == 2), t_horizon)

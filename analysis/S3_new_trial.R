...
compute_enbs <- function (evsi_ar, nb, c_fix, c_var, lag_awr, lag_oir,
  ...
  v_dh_awr <- decision_horizons(evsi_ar$fu, lag_awr, dec_th)
  v_dh_oir <- decision_horizons(evsi_ar$fu, lag_oir, dec_th)
...
df_enbs <- compute_enbs(
  ...
  c_fix     = 1000000 / thresh,         # Assumed fixed costs
  c_var     = 400000 / thresh,          # Assumed monthly variable cost
  lag_awr   = 9,                        # AWR lag assumption
  lag_oir   = 6,                        # OIR lag assumption

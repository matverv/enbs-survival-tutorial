...
# Benefit of Sampling (ENBS) for a new trial.
#
...
# New sample sizes for each treatment and survival outcome
v_n <- rep(200, times = 4)
...
# Survival data generation function for a new trial
sim_surv_data_new <- function(surv_probs, n, enroll_pars, dropout_pars, 
  ...
  # n:               Sample size for the treatment and survival outcome in `surv_probs`.
  # enroll_pars:     Gamma parameters for monthly enrollment rate.                  
  # dropout_pars:    Gamma parameters for monthly dropout rate (dropouts, time at risk).
  ...
  # Sample enrollment rates from a gamma distribution
  v_enroll_rates <- rgamma(n = n_sim, shape = enroll_pars[1], rate = enroll_pars[2])
  
  # Generate a matrix of enrollment times from exponential distributions
  m_enroll_times <- sapply(v_enroll_rates, function(enroll_rate) {
    cumsum(rexp(n = n, rate = enroll_rate)) # Take cumulative sum of inter-enrollment times
  })
  
  # Generate 'n' random numbers uniformly distributed between survival probabilities 
  # corresponding the model time horizon and 1
  ...
  # Adjust survival times by subtracting enrollment times, and ensure no negative times.
  m_surv_times <- pmax(m_surv_times - m_enroll_times, 0)
  ...
  for (i in seq_along(adm_cens_months)) {
    ...
    m_cens_times <- pmin(m_dropout_times, adm_cens_months[i])
    ...
    df_summ_stat <- data.frame(
      ...
      timeRisk = colSums(pmin(m_surv_times, m_cens_times))
    )
...
l_summ_stat <- Map(f = sim_surv_data_new, 
                   l_surv, v_n, l_enroll, l_dropout_pars, 
...
# NOTE: The following steps are relevant for event-driven trials but not for fixed-duration/time-driven trials
# Define target number of total events for trial completion (assumption)
events_target <- 200

# Calculate mean predicted numbers of events across follow-up times for both outcomes (OS1 and OS2)
v_events <- sapply(l_summ_stat, function(df) mean(df$os1.events + df$os2.events))

# Estimate required follow-up duration (in months) to reach target event count
round(spline(v_events, v_adm_cens_months, xout = events_target)$y)
...
# or using a faster method such as "earth" (method = "earth") or "bart",
...
compute_enbs <- function (evsi_ar, nb, c_fix, c_var, lag_awr, lag_oir,
  ...
  #           standard care).
  # c_fix:    Fixed trial cost.
  # c_var:    Monthly trial conduct cost.
  # lag_awr:  Time between end of follow-up and change of practice for AWR.
  # lag_oir:  Time between end of follow-up and change of practice for OIR.
  # inc_pop:  Monthly incidence of patients affected by the decision.
  # prev_pop: Prevalent patient population affected by the decision.
  # dec_th:   Decision relevance horizon in months.
  ...
  pv_annuity <- function(t, dr_monthly) {
    if (dr_monthly > 0) {
      return((1 - (1 + dr_monthly)^-t) / dr_monthly)
    } else {
      return(t)
    }
  }
  ...
  dr_monthly <- (1 + dr)^(1/12) - 1
  
  # --- OIR Calculation ---
  # Total time until research is implemented
  t_research_oir <- pmin(evsi_ar$fu + lag_oir, dec_th)
  # Time remaining in horizon for benefits to accrue
  t_post_research_oir <- pmax(dec_th - t_research_oir, 0)
  
  # Value of the benefit stream at the time it starts (future value)
  pop_evsi_oir_fv <- (prev_pop + inc_pop * pv_annuity(t_post_research_oir, dr_monthly)) * evsi_ar$evsi
  # Discount the benefit stream back to present value
  pop_evsi_oir_pv <- pop_evsi_oir_fv / (1 + dr_monthly)^t_research_oir
  
  # Opportunity cost of not choosing the best immediate option during the research period
  m_inb_new <- nb[, -1, drop = FALSE] - nb[, 1]
  inb_best_new <- max(c(colMeans(m_inb_new), 0))
  pop_during_research_oir <- prev_pop + inc_pop * pv_annuity(t_research_oir, dr_monthly)
  opp_cost_oir <- inb_best_new * pop_during_research_oir
  
  # Total research costs (fixed + discounted variable costs)
  research_costs <- c_fix + c_var * pv_annuity(evsi_ar$fu, dr_monthly)
  
  # ENBS for OIR
  v_enbs_oir <- pop_evsi_oir_pv - opp_cost_oir - research_costs
  
  # --- AWR Calculation ---
  # Total time until research is implemented
  t_research_awr <- pmin(evsi_ar$fu + lag_awr, dec_th)
  # Time remaining in horizon for benefits to accrue
  t_post_research_awr <- pmax(dec_th - t_research_awr, 0)
  
  # Value of the benefit stream at the time it starts (future value)
  pop_evsi_awr_fv <- (prev_pop + inc_pop * pv_annuity(t_post_research_awr, dr_monthly)) * evsi_ar$evsi * prob_rev
  # Discount the benefit stream back to present value
  pop_evsi_awr_pv <- pop_evsi_awr_fv / (1 + dr_monthly)^t_research_awr
  
  # Opportunity cost of implementing a potentially suboptimal treatment during research
  # Note: This is calculated per treatment, but the example uses a single new treatment
  inb_treatment <- nb[, 2] - nb[, 1] # Assuming column 2 is the new treatment vs col 1 standard care
  opp_cost_awr <- pmin(mean(inb_treatment), 0) * (prev_pop + inc_pop * pv_annuity(t_research_awr, dr_monthly))
  
  # ENBS for AWR
  v_enbs_awr <- pop_evsi_awr_pv - abs(opp_cost_awr) - research_costs
...
df_enbs <- compute_enbs(
  ...
  lag_awr   = 12 - 9,                  # Adjusted AWR lag assumption
  lag_oir   = 9 - 9,                   # Adjusted OIR lag assumption
